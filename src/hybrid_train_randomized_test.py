...
def train_model(graph, graph_by_type, args):
    ...
    pickle_name = '_'.join([HybridGNN.name, str(args.input).split("/")[-1], str(args.filter), str(args.num_walks), str(args.walk_length),
    ...
    for epoch in range(args.epoch):
        ...
        for i, data in enumerate(data_iter):
            ...
            embs = model(nodeid.to(device), (rows, typeid.to(device)),
                         meta_neigh.to(device), rand_neigh.to(device) if not args.rm_rand else None)
        ...
        for i in range(len(all_nodes)):
            ...
            node_emb = model(train_inputs, (rows, train_types), node_neigh, rand_node_neigh if not args.rm_rand else None)
