...
def train_model(graph, graph_by_type, args):
    ...
    pickle_name = '_'.join([HybridGNN.name, str(args.input).split("/")[-1], str(args.maintype), ''.join(map(str, args.upto)), str(args.num_walks), str(args.walk_length),
                   str(args.random_walk_length), str(args.random_sample_layers),
                   str(args.schema).replace(' ', ''), str(args.window_size), '.pkl'])
    ...
    for epoch in range(args.epoch):
        ...
        sperate_eval_auc = {i: 0.0 for i in range(args.edge_type_count)}
        test_sperate_eval_auc = {i: 0.0 for i in range(args.edge_type_count)}
        print("EVAL ON ", args.eval_type.split(","))
        for i in range(args.edge_type_count):
            if args.eval_type == "all" or str(edgetypes[i]) in args.eval_type.split(","):
                ...
                sperate_eval_auc[i] = [tmp_auc, tmp_pr, tmp_f1]
                ...
                test_sperate_eval_auc[i] = [tmp_auc, tmp_pr, tmp_f1]
        ...
        if cur_score > best_score:
            ...
            patience = 0
            torch.save(copy.deepcopy(model.state_dict()), model.name + '.chkpt')
            # model.load_state_dict(torch.load(model.name + '.chkpt'))
        else:
            patience += 1
            if patience > args.patience:
                print("Early Stopping")
                print("========> SEPERATION LINE")
                print(sperate_eval_auc)
                print(test_sperate_eval_auc)
                print("========> SEPERATION LINE ENDING.")
...
if __name__ == "__main__":
    ...
    training_graphs, training_data_by_type = load_training_data_single(
        file_name + train_txt,
        main=args.maintype,
        upto=args.upto,
    )
    valid_true_data_by_edge, valid_false_data_by_edge = load_testing_data_single(
        file_name + valid_txt,
        main=args.maintype,
        upto=args.upto,
    )
    testing_true_data_by_edge, testing_false_data_by_edge = load_testing_data_single(
        file_name + test_txt,
        main=args.maintype,
        upto=args.upto,
    )
